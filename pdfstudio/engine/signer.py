...
class SignatureConfig:
    ...
    sig_type: str = "typed"
    image_bytes: Optional[bytes] = None       # For drawn/image signatures
    cert_path: Optional[str] = None           # Path to .p12 / .pfx
...
class SignatureEngine:
    ...
    def sign_visual(self, pdf_path: str | Path, output_path: str | Path,
                    config: SignatureConfig) -> bool:
        """
        ...
        try:
            ...
            if config.image_bytes:
                ...
                page.insert_textbox(
                    rect, config.signer_name,
                    fontsize=14, color=(0.1, 0.1, 0.5),
                    align=fitz.TEXT_ALIGN_CENTER,
                )
                # Signature line
                y = rect.y1 - 4
                page.draw_line(fitz.Point(rect.x0 + 4, y), fitz.Point(rect.x1 - 4, y),
    ...
    def sign_cryptographic(self, pdf_path: str | Path, output_path: str | Path,
                           config: SignatureConfig) -> bool:
        """
        ...
        """
        try:
            from pyhanko.sign import signers, fields as hanko_fields
