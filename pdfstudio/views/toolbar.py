...
from PySide6.QtWidgets import (
    QToolBar, QLabel, QSpinBox, QComboBox, QWidget, QSizePolicy,
)
...
class MainToolbar(QToolBar):
    ...
    def _build(self) -> None:
        ...
        # ── Form fields ─────────────────────────────────────────────────
        for mode in (ToolMode.TEXT_FIELD, ToolMode.CHECKBOX, ToolMode.RADIO,
                     ToolMode.DROPDOWN, ToolMode.LISTBOX,
                     ToolMode.SIGNATURE_FIELD, ToolMode.BUTTON):
            self._add_tool(mode, checkable=True)
        self.addSeparator()

        # ── Annotations ─────────────────────────────────────────────────
        for mode in (ToolMode.HIGHLIGHT, ToolMode.UNDERLINE, ToolMode.STRIKEOUT,
                     ToolMode.NOTE, ToolMode.TEXT_BOX, ToolMode.INK,
                     ToolMode.RECTANGLE, ToolMode.ELLIPSE,
                     ToolMode.LINE, ToolMode.ARROW, ToolMode.STAMP):
            self._add_tool(mode, checkable=True)
        self.addSeparator()
        ...
        self._zoom_combo.addItems(["50%", "75%", "100%", "125%", "150%",
                                   "200%", "300%", "Fit Page", "Fit Width"])
    ...
    def _add_tool(self, mode: ToolMode, checkable: bool = False,
